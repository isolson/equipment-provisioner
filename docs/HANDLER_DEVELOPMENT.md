...
| `supported_post_provision_modes` | `()` | Declares the deployment modes that the operator may apply after infrastructure provisioning. The API and kiosk derive AP/PTP actions from this tuple; shared code must not maintain a vendor allowlist. Cambium and Tachyon: `("ap", "ptp")` |
