...
class PortManager:
    ...
    async def _bootp_listener_loop(self, port_num: int, interface: str) -> None:
        ...
        while self._running:
            ...
            # different MAC. This prevents a stuck reset/BOOTP unit from
            # looping destructive flash attempts.
            if (
                state.last_bootp_fired_at is not None
                and state.last_bootp_fired_mac is not None
                and mac.lower() == state.last_bootp_fired_mac.lower()
            ):
                elapsed = int(time.time() - state.last_bootp_fired_at)
                logger.info(
                    f"Auto-Netinstall: BOOTP from {mac} on port {port_num} ignored — "
                    f"already attempted {elapsed}s ago; remove/reinsert device before retry"
                )
                continue
    ...
    def get_available_ptp_side(
        ...
        When a port already owns one side of this link, preserve that side so
        an operator can safely reapply a corrected profile.  Without the
        port hint, the historical auto-assignment behavior is unchanged.
        ...
        # The link registry is in-memory and may be rebuilt later than the
        # per-port state.  Prefer the port's recorded side when available so
        # a restart followed by a corrective reapply cannot silently swap
        # Main and SM.
        ...
        if link_data.get("side_a_port") is None:
            return "a"
        return "b"
