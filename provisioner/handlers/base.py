...
class BaseHandler(ABC):
    ...
    Each device type implements this interface to provide provisioning
    functionality.
    """
    ...
    def firmware_lookup_key(self, device_info: Optional[DeviceInfo]) -> Optional[str]:
        """Return the key used to look up a model-specific firmware file.

        Defaults to the discovered model, which is what
        ``MODEL_FIRMWARE_PATTERNS`` is keyed on for every vendor whose
        firmware filenames carry the product name.

        Override in handlers whose firmware filenames are keyed on something
        else. Note that several vendors populate ``hardware_version`` for
        display purposes without it being the firmware key — do **not**
        broaden this default to prefer ``hardware_version``, or those vendors
        will stop matching any firmware file.
    ...
    async def upload_firmware(self, firmware_path: str, bank: Optional[int] = None) -> bool:
        ...
        argument. Some vendors use a different endpoint for the second-pass
        flash and need this to switch paths.
