...
def _client(tmp_path, monkeypatch):
    monkeypatch.setattr("provisioner.handlers.cambium.CambiumHandler._get_custom_credential", lambda self: None)
    config = Config()
...
def test_host_credentials_resolves_management_target_and_edits_rw_without_echo(tmp_path, monkeypatch):
    client, config = _client(tmp_path, monkeypatch)
    ...
    response = client.put("/api/host-credentials/cambium", json={"snmp_write_community": "test-write-community"})
    assert response.status_code == 200
    assert "test-write-community" not in response.text
    import yaml
