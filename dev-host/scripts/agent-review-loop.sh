...
SHARED="${AGENT_LOOP_SCRIPT:-/Users/serveradmin/repos/personal-processes/scripts/agent-review-loop.sh}"
...
if [[ ! -x "$SHARED" ]]; then
    echo "Shared agent loop not found or not executable: $SHARED" >&2
    exit 1
fi
