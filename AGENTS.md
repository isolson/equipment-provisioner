...
The vendor list already exists in: `DeviceType` enum, `HANDLER_MAP`, `handlers/__init__.py`, `web/api.py` `BUILTIN_CREDENTIALS`, `index.html` vendor map, `config.py` (`CredentialsConfig`, `DeviceIPsConfig`, firmware sources, feature flags), `port_manager.py` `DeviceLinkLocalIP`, `firmware_checker.py` `SOURCE_MAP` (+ `firmware_sources/__init__.py` imports), and `setup_tools.py`'s readiness/hint/mode dicts. (The CLI, API device-type validation, and setup device-type list derive from `HANDLER_MAP` via `handler_manager.provisionable_device_types()` — Story 2 / #72.) Credentials alone have **four** copies. When you need "the list of vendors," **derive it from an existing registry** (prefer `HANDLER_MAP`/`DeviceType`) — do not hardcode a new list, dict, or `if device_type == "..."`. The target end-state is a single `VendorSpec` registry (see the epic); move toward it, never away.
